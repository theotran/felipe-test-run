...
class AddEmployeeInfo extends Component {
  ...
  handleEmailChange(e) {
    if(!validator.isEmail(e.target.value)){
      this.setState({field: "email", error: "Enter a valid email."})
    } else {
      const newEmails = update(this.state, {emails: {$set: [e.target.value]}});
      newEmails.field = "";
      newEmails.error = "";
  ...
  handleSave(e) {
    ...
    const {firstName, lastName, phones, emails, title, dob, ssn, employeeNumber, employeeStatus, startDate } = this.state;
    let employeeGroupId;
    for(let k=0; k < this.props.data.company.groups.length; k++){
      if(this.props.data.company.groups[k].name === 'Employee' && this.props.data.company.groups[k].type === 'Person'){
        employeeGroupId = this.props.data.company.groups[k].id;
        break;
      }
    }

    this.props.CreateNewPerson({variables: { firstName, lastName, phones, emails }}).then((person) => {
      if(person.data.createPerson.success === false){
        ...
        this.props.CreateEmployeeInfo({
          ...
          console.log('AddEmployeeInfo', employeeInfo);
          if(employeeInfo.data.createEmployeeInfo.success !== true){
            this.setState({
              field: employeeInfo.data.createEmployeeInfo.field,
              error: employeeInfo.data.createEmployeeInfo.message
            })
          } else {
              ...
              if(group.data.addToGroup.success === false){
                ...
                this.props.AddAddress({
                  ...
                  phones.map((phone, index) => {
                    this.props.CreateNewPhone({variables: {type: "", number: phone, belongsTo: "Person", belongsToId: person.data.createPerson.id}}).then((phone) => {
                  ...
                  emails.map((email, index) => {
                    this.props.CreateNewEmail({variables: {type: "", address: email, belongsTo: "Person", belongsToId: person.data.createPerson.id}}).then((email) => {
                  ...
                  browserHistory.push(`/employees/${person.data.createPerson.id}`);
                })
              }
            })
          }
  ...
  render() {
    ...
    return (
      ...
      <div>
        <div className='row'>
          <Button color="teal" content='ADD EMPLOYEE' icon='plus' labelPosition='left' onClick={(e, data) => {this.handleOpen()}}/>
        </div>
        <Dialog
          title="Add Employee"
          modal={true}
        ...
        >
          ...
          <form onSubmit={this.handleSave}>
            ...
            <div className="row">
              <div className="form-group col-sm-6">
                <DatePicker errorText={errorDOB} hintText="Date of Birth" autoOk={true} floatingLabelFixed={true} floatingLabelText="Date of Birth" id="2" value={this.state.dob ? moment(this.state.dob)._d : {}} onChange={(e, date) => this.setState({ dob: moment(date)._d})} />
              </div>

              <div className="form-group col-sm-6">
                <DatePicker errorText={errorStartDate} hintText="Start Date" autoOk={true} floatingLabelFixed={true} floatingLabelText="Start Date" id="2" value={this.state.startDate ? moment(this.state.startDate)._d : {}} onChange={(e, date) => this.setState({ startDate: moment(date)._d})} />
              </div>
            </div>
...
const AddEmployeeInfoWithMutation = compose(
  graphql(createPersonMutation, { name: "CreateNewPerson" }),
  graphql(createEmployeeInfoMutation, { name: "CreateEmployeeInfo" }),
  graphql(createPhoneMutation, { name: "CreateNewPhone" }),
  graphql(createEmailMutation, { name: "CreateNewEmail" }),
  graphql(AddToGroupMutation, { name: "AddToGroup" }),
  graphql(AddAddressMutation, { name: "AddAddress" }),
  graphql(GetCompanyGroups)
  )(withRouter(AddEmployeeInfo));

export default AddEmployeeInfoWithMutation;
